...
const HRList = ({ token }) => {
  ...
  const fetchHRList = async () => {
    try {
      const res = await axios.get("http://https://employee-analysis-system-1.onrender.com//hr", {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await axios.put(`http://https://employee-analysis-system-1.onrender.com//hr/${editId}`, formData, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setEditId(null);
      } else {
        await axios.post("http://https://employee-analysis-system-1.onrender.com//hr", formData, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://https://employee-analysis-system-1.onrender.com//hr/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchHRList();
    } catch (err) {
